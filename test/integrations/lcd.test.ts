...
const PATH_TO_QUERY_TYPES = './node_modules/@kyve/proto/dist/proto/kyve/registry/v1beta1/query'
...
beforeAll(async () => {
    ...
    typeQuerySchemas = TJS.buildGenerator(program, settings) as unknown as JsonSchemaGenerator;
...
function validate(schema: Definition, data: any): { valid: boolean; errors: ErrorObject[] | null } {
...
it('Query <fundersList>',async () => {
    const poolsResponse = await lcdClient.kyve.pools()
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryFundersListResponse");
    for(let pool of poolsResponse.pools) {
        const poolsResponse = await lcdClient.kyve.fundersList({pool_id: pool.id})
        const vResult = validate(schema, poolsResponse)
        expect(vResult.valid).toBeTruthy()
    }
})
it('Query <funder>', async () => {
    ...
    const founders = await lcdClient.kyve.fundersList({pool_id: pool.pools[0].id})
    const founder = await lcdClient.kyve.funder({pool_id: pool.pools[0].id, funder: founders.funders[0].account})
...
it('Query <stakersList>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakers = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryStakersListResponse");
...
it('Query <staker>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const stakerResponse = await lcdClient.kyve.staker({pool_id: pool.pools[0].id,  staker: stakersListResponse.stakers[0].staker})
...
it('Query <proposals>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const proposals = await lcdClient.kyve.proposals({pool_id: pool.pools[0].id})
...
it('Query <proposal>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const proposals = await lcdClient.kyve.proposals({pool_id: pool.pools[0].id})
    const proposal = await lcdClient.kyve.proposal({ bundle_id: proposals.proposals[0].bundle_id })
...
it('Query <proposalByHeight>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const proposalByHeightResponse = await lcdClient.kyve.proposalByHeight({ pool_id: pool.pools[0].id, height: TEST_HEIGHT })
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryProposalByHeightResponse");
...
it('Query <canVote>', async () => {
    ...
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const proposals = await lcdClient.kyve.proposals({pool_id: pool.pools[0].id})
    const canVoteRes = await lcdClient.kyve.canVote({pool_id: pool.pools[0].id, voter: stakersListResponse.stakers[0].staker, bundle_id:proposals.proposals[0].bundle_id})
...
it('Query <stakeInfo>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const stakeInfoRes = await lcdClient.kyve.stakeInfo({pool_id: pool.pools[0].id, staker: stakersListResponse.stakers[0].staker});
...
it('Query <accountAssets>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const accountAssetsRes = await lcdClient.kyve.accountAssets({address: stakersListResponse.stakers[0].account})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryAccountAssetsResponse");
...
it('Query <accountFundedList>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const foundersRes = await lcdClient.kyve.fundersList({pool_id: pool.pools[0].id})
    const accountFundedListRes = await lcdClient.kyve.accountFundedList({address: foundersRes.funders[0].account})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryAccountFundedListResponse");
    // do not test pagination property
    delete schema.properties?.pagination;
...
it('Query <accountStakedList>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const accountStakedListRes = await lcdClient.kyve.accountStakedList({address: stakersListResponse.stakers[0].account})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryAccountStakedListResponse");
    // do not test pagination property
    delete schema.properties?.pagination;
...
it('Query <accountDelegationList>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const accountDelegationListRes = await lcdClient.kyve.accountDelegationList({address: stakersListResponse.stakers[0].account})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryAccountDelegationListResponse");
    // do not test pagination property
    delete schema.properties?.pagination;
...
it('Query <delegatorsByPoolAndStaker>', async () => {
    const pool = await lcdClient.kyve.pools({pagination: {limit: '1'}})
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const delegatorsRes = await lcdClient.kyve.delegatorsByPoolAndStaker({pool_id: pool.pools[0].id, staker: stakersListResponse.stakers[0].staker})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryDelegatorsByPoolAndStakerResponse");
    //do not test pagination property
    delete schema.properties?.pagination;
...
it('Query <stakersByPoolAndDelegator>', async () => {
    ...
    const stakersListResponse = await lcdClient.kyve.stakersList({pool_id: pool.pools[0].id})
    const delegatorsRes = await lcdClient.kyve.delegatorsByPoolAndStaker({pool_id: pool.pools[0].id, staker: stakersListResponse.stakers[0].staker})
    const stakersByPoolAndDelegatorRes = await lcdClient.kyve.stakersByPoolAndDelegator({pool_id: pool.pools[0].id, delegator: delegatorsRes.delegators[0].delegator})
    const schema = typeQuerySchemas.getSchemaForSymbol("QueryStakersByPoolAndDelegatorResponse");
    //do not test pagination property
    delete schema.properties?.pagination;
